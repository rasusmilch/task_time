...
class TagSelectionFrame(ttk.Frame):
    ...
    def __init__(self, parent: ttk.Widget, service: object, initial_tags: Iterable[str], allow_new_tags: bool = True) -> None:
        ...
        self.selected_list.bind("<Double-Button-1>", lambda _e: self._remove_selected_assigned())

        actions = ttk.Frame(self)
        actions.grid(row=1, column=0, sticky="ew", pady=(6, 0))
        if allow_new_tags:
            ttk.Button(actions, text="Add New Tag", command=self._prompt_add_new_tag).pack(side="left")
    ...
    @staticmethod
    def _sorted_visible_tags(global_tags: Iterable[object], selected_tags: Iterable[str]) -> list[str]:
        selected = set(selected_tags)
        keys = [meta.key for meta in global_tags if not meta.archived and meta.key not in selected]
        return sorted(keys)
