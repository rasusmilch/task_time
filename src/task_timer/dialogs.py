    ...
    def __init__(
        ...
        *,
        ...
        self.date_var = StringVar(
            value=(initial_date or datetime.now(service.local_tz).date()).isoformat()
        )
        ...
        if running_start_label:
            ttk.Label(self.window, text=f"Running since: {running_start_label}").grid(
                row=row, column=0, columnspan=2, padx=8, pady=(0, 4), sticky="w"
            )
            row += 1
        ...
        if DateEntry is not None:
            ...
            self.date_widget.set_date(
                (initial_date or datetime.now(service.local_tz).date()).isoformat()
            )
        else:
        ...
        ttk.Entry(self.window, textvariable=self.reason_var).grid(
            row=row, column=1, padx=(0, 8), pady=2, sticky="ew"
        )
        row += 1

        ttk.Label(self.window, textvariable=self.error_var, foreground="#b00020").grid(
            row=row, column=0, columnspan=2, padx=8, pady=(2, 4), sticky="w"
        )
        row += 1
    ...
    def _confirm(self) -> None:
        try:
            ...
            if self.force_fix_stop:
                ...
                stop_local = self.service.parse_local_datetime_inputs(
                    work_date, self.stop_var.get().strip()
                )
                self.result = TimelineEntryResult(
                    "fix_stop", work_date, None, stop_local, None, reason
                )
            elif self.mode_var.get() == "duration":
                if not self.duration_var.get().strip():
                    raise ValueError("Duration is required")
                duration_seconds = self.service.parse_duration_input_seconds(
                    self.duration_var.get().strip()
                )
                self.result = TimelineEntryResult(
                    "duration", work_date, None, None, duration_seconds, reason
                )
            else:
                if not self.start_var.get().strip() or not self.stop_var.get().strip():
                    raise ValueError("Start and stop are required")
                start_local = self.service.parse_local_datetime_inputs(
                    work_date, self.start_var.get().strip()
                )
                stop_local = self.service.parse_local_datetime_inputs(
                    work_date, self.stop_var.get().strip()
                )
                ...
                self.result = TimelineEntryResult(
                    "start_end", work_date, start_local, stop_local, None, reason
                )
...
class EditTimelineDialog:
    ...
    def __init__(
        ...
        self.tz_label_var = StringVar(
            value=f"Times shown in local timezone: {self.service.local_tz_name}"
        )
        ttk.Label(self.window, textvariable=self.tz_label_var).grid(
            row=0, column=0, sticky="e", padx=6, pady=(6, 2)
        )
        ...
        ttk.Button(button_bar, text="Add interval", command=self._add_interval).pack(
            side="left", padx=2
        )
    ...
    def _refresh_table(self) -> None:
        ...
        rows = self.service.get_task_timeline(
            self.task_id, include_before_reset=self.include_history_var.get()
        )
        for row in rows:
            iid = row["interval_id"]
            self.tree.insert(
    ...
    def _add_interval(self) -> None:
        try:
            entry = TimelineEntryDialog(
                ...
                default_mode="start_end",
            ).result
            if not entry:
                return
            if entry.mode == "start_end" and entry.start_local and entry.stop_local:
                self.service.add_manual_interval(
                    self.task_id, entry.start_local, entry.stop_local, entry.reason
                )
            elif entry.mode == "duration" and entry.duration_seconds is not None:
                self.service.add_manual_duration(
                    self.task_id, entry.work_date, entry.duration_seconds, entry.reason
                )
            else:
    ...
    def _edit_selected(self) -> None:
        try:
            ...
            entry = TimelineEntryDialog(
                ...
                initial_date=(
                    date.fromisoformat(interval.work_date_local)
                    if interval.work_date_local
                    else start_local.date()
                ),
                initial_start_text=start_local.strftime("%I:%M %p").lstrip("0"),
                initial_stop_text=stop_local.strftime("%I:%M %p").lstrip("0"),
                initial_duration_text=format_duration_hm(
            ...
            if not entry:
                return
            if entry.mode == "duration" and entry.duration_seconds is not None:
                self.service.edit_duration_interval(
                    self.task_id,
                    interval_id,
                    entry.work_date,
    ...
    def _delete_selected(self) -> None:
        try:
            ...
            if not messagebox.askyesno(
                "Confirm delete",
                "Delete selected interval from totals?",
                parent=self.window,
            ):
                return
...
class AddTaskDialog:
    ...
    def __init__(self, parent: Toplevel, service: "TaskTimerService") -> None:
        ...
        ttk.Label(self.window, text="Task name").grid(
        ...
        ttk.Label(self.window, text="Task note").grid(
            row=1, column=0, padx=(6, 4), pady=2, sticky="w"
        )
        ...
        ttk.Button(button_row, text="Cancel", command=self.window.destroy).pack(
            side="right", padx=4
        )
...
class SubtaskTemplateSelectionFrame(ttk.LabelFrame):
    def __init__(self, parent: tk.Misc, service: "TaskTimerService") -> None:
        ...
        list_canvas.bind(
            "<Configure>",
            lambda e: list_canvas.itemconfigure(list_window_id, width=e.width),
        )
        for template in self.templates:
            ...
            ttk.Checkbutton(
                ...
                text=template.name,
                variable=var,
                command=self._refresh_selected_label,
            ).pack(anchor="w")
    ...
    def _refresh_selected_label(self) -> None:
        names = [
            template.name
            for template in self.templates
            if self._template_vars[template.template_id].get()
        ]
...
class BackupSettingsDialog:
    ...
    def __init__(
        ...
        ttk.Entry(self.window, textvariable=self.son_days_var).grid(
            row=0, column=1, padx=(0, 6), pady=2, sticky="ew"
        )
        ttk.Label(self.window, text="Father retention days").grid(
            row=1, column=0, padx=(6, 4), pady=2, sticky="w"
        )
        ttk.Entry(self.window, textvariable=self.father_days_var).grid(
            row=1, column=1, padx=(0, 6), pady=2, sticky="ew"
        )
        ttk.Label(self.window, text="Grandfather retention days").grid(
            row=2, column=0, padx=(6, 4), pady=2, sticky="w"
        )
        ttk.Entry(self.window, textvariable=self.grandfather_days_var).grid(
            row=2, column=1, padx=(0, 6), pady=2, sticky="ew"
        )
        ttk.Checkbutton(
            self.window,
        ...
        ttk.Entry(self.window, textvariable=self.min_interval_var).grid(
            row=5, column=1, padx=(0, 6), pady=2, sticky="ew"
        )
...
class MonthEndReminderSettingsDialog:
    ...
    def __init__(self, parent: Toplevel, initial: UISettings) -> None:
        ...
        ttk.Checkbutton(
            self.window,
            text="Enable month-end time-entry reminder",
            variable=self.enabled_var,
        ).pack(fill="x", padx=10, pady=(10, 4))
        ttk.Checkbutton(
            self.window,
            text="Show reminder when app starts on reminder day",
            variable=self.startup_var,
        ).pack(fill="x", padx=10, pady=4)
        ttk.Checkbutton(
            self.window,
            text="Show reminder when closing app on reminder day",
...
class SelectedTaskExportDialog:
    def __init__(self, parent: Toplevel, service: "TaskTimerService") -> None:
        ...
        ttk.Radiobutton(
            frame,
            text="Since last export checkpoint",
        ...
        ttk.Entry(dates, textvariable=self.end_date_var).grid(
        ...
        roots = sorted(
            [t for t in active_tasks if t.parent_task_id is None],
            key=lambda t: (t.name.strip().casefold(), t.task_id),
        )
        children_by_parent = {
            t.task_id: sorted(
                service.child_tasks(t.task_id),
                key=lambda c: (c.name.strip().casefold(), c.task_id),
            )
            for t in roots
        }
        for task in roots:
        ...
        ttk.Entry(reason_row, textvariable=self.reason_var).pack(
    ...
    def validate_inputs(
        *,
        ...
        return SelectedTaskExportResult(
...
class MoveTaskDialog:
    def __init__(
        ...
        row = 0
        ttk.Label(self.window, text=f"Task: {task.name}").grid(
            row=row, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 2)
        )
        row += 1
        ttk.Label(self.window, text=f"Current location: {current_location}").grid(
        ...
        self.parent_radio = ttk.Radiobutton(
            self.window,
            text="Move under parent task",
            value="parent",
            variable=self.mode_var,
            command=self._on_mode_change,
        )
...
class EditTaskDialog:
    def __init__(
        ...
        if self.task_depth <= 1:
            ...
            subtask_scroll = ttk.Scrollbar(
                subtask_frame, orient="vertical", command=self.subtask_tree.yview
            )
            ...
            ttk.Button(
                ...
                command=self._edit_selected_subtask,
            ).pack(side="left", padx=4)
            ttk.Button(
                subtask_buttons,
                text="Delete Selected Subtask",
                command=self._delete_selected_subtask,
            ).pack(side="left", padx=4)
            if self.task_depth <= 1:
                ttk.Button(
                    subtask_buttons,
            ...
            ttk.Label(self.window, text="Maximum subtask depth reached.").grid(
    ...
    def _move_task(self) -> None:
        ...
        if dialog.new_parent_task_id is not None and not self.service.state.tasks.get(
            ...
            messagebox.showerror(
                "Move Task",
                "Selected parent task no longer exists.",
                parent=self.window,
            )
            return
    ...
    def _refresh_subtasks(self) -> None:
        ...
        for child in self.service.child_tasks(self.task_id, include_deleted=False):
            self.subtask_tree.insert(
                ...
                "end",
                iid=child.task_id,
                values=(child.name, child.notes, str(len(child.tags))),
            )
    ...
    def _apply_subtask_templates(self) -> None:
        ...
        if result.created_subtask_ids:
            ...
            messagebox.showinfo(
                ...
                "No subtasks were created. All template items already exist under this task.",
                parent=self.window,
            )
    ...
    def _delete_selected_subtask(self) -> None:
        try:
            ...
            return
        if not messagebox.askyesno(
            "Delete Subtask", "Delete selected subtask?", parent=self.window
        ):
            return
...
class ManageSubtaskTemplatesDialog:
    def __init__(self, parent: Toplevel, service: "TaskTimerService") -> None:
        ...
        self.template_list = tk.Listbox(
            template_list_frame, exportselection=False, height=16, width=30
        )
        template_list_scroll = ttk.Scrollbar(
            template_list_frame, orient="vertical", command=self.template_list.yview
        )
        ...
        ttk.Entry(right, textvariable=self.template_name_var).grid(
            row=1, column=0, sticky="ew", pady=(0, 4)
        )
        ttk.Label(right, text="Template notes").grid(row=2, column=0, sticky="w")
        ttk.Entry(right, textvariable=self.template_notes_var).grid(
        ...
        self.item_tree = ttk.Treeview(
            tree_frame,
            columns=("order", "name", "notes", "tags"),
            show="tree headings",
            height=10,
        )
        self.item_tree.heading("#0", text="Hierarchy")
        self.item_tree.column("#0", width=180, anchor="w")
        for c, t, w in [
            ("order", "Order", 50),
            ("name", "Name", 180),
            ("notes", "Notes", 220),
        ...
        item_tree_scroll = ttk.Scrollbar(
        ...
        btns.grid(row=5, column=0, sticky="w", pady=4)
        for txt, cmd in [
            ("Add Subtask", self._add_item),
            ("Add Nested Subtask", self._add_nested_item),
    ...
    def _current(self):
        return next(
            (t for t in self.templates if t.template_id == self.current_template_id),
            None,
        )
    ...
    def _on_select_template(self):
        if self.dirty and self.current_template_id:
            ans = messagebox.askyesnocancel(
                "Unsaved Changes",
                "Save changes before switching template?",
                parent=self.window,
            )
            if ans is None:
                return
        ...
        if not sel:
            return
        self.current_template_id = self.templates[int(sel[0])].template_id
        self._load_current()
        self.dirty = False
    ...
    def _load_current(self):
        ...
        if not t:
            return
        self.template_name_var.set(t.name)
        self.template_notes_var.set(t.notes)
        ...
        for it in roots:
            self.item_tree.insert(
                ...
                iid=it.item_id,
                text="Subtask",
                values=(order, it.name, it.notes, ", ".join(it.tags)),
            )
            order += 1
            for child in [c for c in children if c.parent_item_id == it.item_id]:
                self.item_tree.insert(
                    ...
                    iid=child.item_id,
                    text="Nested Subtask",
                    values=(order, child.name, child.notes, ", ".join(child.tags)),
                )
                order += 1
    ...
    def _save_current(self):
        ...
        if not t:
            return
        try:
            self.service.update_subtask_template(
                t.template_id,
                self.template_name_var.get().strip(),
                self.template_notes_var.get().strip(),
                self._items_from_tree(),
            )
        except Exception as exc:
            messagebox.showerror("Save Template", str(exc), parent=self.window)
            return
        self.templates = self.service.list_subtask_templates()
        self.changed = True
        self.dirty = False
        self._refresh_template_list()
    ...
    def _add_template(self):
        try:
            tid = self.service.create_subtask_template("New Template", "")
        except Exception as exc:
            messagebox.showerror("Add Template", str(exc), parent=self.window)
            return
        self.templates = self.service.list_subtask_templates()
        self.changed = True
        self._refresh_template_list()
    ...
    def _delete_template(self):
        ...
        if not t:
            return
        if not messagebox.askyesno(
            "Delete Template", f"Delete template '{t.name}'?", parent=self.window
        ):
            return
        self.service.delete_subtask_template(t.template_id)
        self.templates = self.service.list_subtask_templates()
    ...
    def _selected_item_id(self):
        ...
        if not s:
            raise ValueError("Select a subtask item first")
        return str(s[0])
    ...
    def _add_nested_item(self):
        ...
        try:
            ...
            messagebox.showerror("Add Nested Subtask", str(exc), parent=self.window)
            return
        parent_item = next(i for i in t.items if i.item_id == iid)
        if parent_item.parent_item_id is not None:
            messagebox.showerror(
                "Add Nested Subtask",
                "Select a top-level subtask item.",
                parent=self.window,
            )
            return
        d = SubtaskTemplateItemDialog(
    ...
    def _edit_item(self):
        ...
        it.notes = d.result["notes"]
        it.tags = d.result["tags"]
        self.dirty = True
        self._load_current()
    ...
    def _remove_item(self):
        ...
        try:
            ...
            messagebox.showerror("Remove Subtask Item", str(exc), parent=self.window)
            return
        if not messagebox.askyesno(
            "Remove Subtask Item", "Remove selected template item?", parent=self.window
        ):
            return
        t.items = [i for i in t.items if i.item_id != iid]
        self.dirty = True
        self._load_current()
    ...
    def _move(self, delta: int):
        ...
        try:
            ...
            return
        idx = next((i for i, x in enumerate(t.items) if x.item_id == iid), -1)
        j = idx + delta
        if idx < 0 or j < 0 or j >= len(t.items):
            return
        t.items[idx], t.items[j] = t.items[j], t.items[idx]
        self.dirty = True
        self._load_current()
        self.item_tree.selection_set(t.items[j].item_id)

    def _close(self):
        if self.dirty:
            ans = messagebox.askyesnocancel(
                "Unsaved Changes", "Save changes before closing?", parent=self.window
            )
            if ans is None:
                return
            if ans:
...
class ManageTagsDialog:
    def __init__(self, parent: Toplevel, service: "TaskTimerService") -> None:
        ...
        self.tree = ttk.Treeview(
            self.window, columns=("tag", "status", "usage"), show="headings", height=12
        )
    ...
    def _rename_tag(self) -> None:
        try:
            ...
            raw_new = simpledialog.askstring(
                "Rename Tag", "New tag key:", initialvalue=old_key, parent=self.window
            )
            if raw_new is None:
                return
    ...
    def _delete_tag(self) -> None:
        try:
            ...
            if not messagebox.askyesno(
                "Delete Tag",
                f"Delete tag '{key}'? This cannot be undone.",
                parent=self.window,
            ):
                return
