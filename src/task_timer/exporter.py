...
from .time_utils import format_duration_hm_and_decimal, to_utc_z
...
def build_export_text(
    *,
    ...
    lines.append("=" * 72)
    lines.append(
        "Note: Tag totals are non-exclusive label totals. If a task has multiple tags, its full time is counted under each tag. Tag totals may exceed overall tracked time."
    )
    for week in sorted(tag_weekly):
        ...
        for tag in sorted(tag_weekly[week], key=lambda t: (t == "untagged", t)):
            ...
            rows.append(
                [
                    tag,
                    format_duration_hm_and_decimal(info["seconds"]),
                    _format_contributing_tasks(info["tasks"]),
                ]
            )
    ...
    for row in per_task_rows:
        ...
        if row["weekly_totals"]:
            ...
            lines.append("    - None")
        lines.append(
            f"  Overall total since checkpoint: {format_duration_hm_and_decimal(row['overall_seconds'])}"
        )
...
def build_selected_tasks_export_text(
    *,
    ...
    lines.append(f"Local timezone: {local_timezone}")
    lines.append(
        f"Window start (exclusive): {to_utc_z(window_start_utc) if window_start_utc else 'Beginning of recorded history'}"
    )
    ...
    for row in weekly_summary_rows:
        rows.append(
            [
                row["name"],
                row["notes"],
                *[format_duration_hm_and_decimal(v) for v in row["weeks"]],
            ]
        )
    lines.extend(
        _render_table(headers, rows)
        if rows
        else ["No selected tasks found for this export window."]
    )
    lines.append("")
