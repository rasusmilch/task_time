...
def test_export_without_prior_checkpoint_uses_full_history(
    ...
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-03T00:00:00Z")
    )
...
def test_export_with_prior_checkpoint_starts_after_checkpoint(
    ...
    service = TaskTimerService(storage)
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-15T00:00:00Z")
    )
...
def test_successful_export_appends_new_checkpoint_event(
    ...
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-10T00:00:00Z")
    )
    ...
    checkpoints = [
        ...
        if e["task_id"] == "__app__" and e["event_type"] == "export_checkpoint"
    ]
    assert checkpoints
...
def test_export_spans_multiple_sunday_start_weeks(tmp_path: Path, monkeypatch) -> None:
    ...
    service = TaskTimerService(storage)
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-16T00:00:00Z")
    )
...
def test_state_not_present_in_export_totals(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-02T00:00:00Z")
    )
...
def test_human_readable_audit_lines_known_events(tmp_path: Path, monkeypatch) -> None:
    ...
    service = TaskTimerService(storage)
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-01T12:00:00Z")
    )
    service.export_report(tmp_path / "out.txt", reset_after=False)
    text = (tmp_path / "out.txt").read_text(encoding="utf-8")
...
def test_reset_after_export_still_resets_and_keeps_checkpoint_behavior(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    task_id = service.create_task("Task A", "")
...
def test_manual_duration_checkpoint_validation_and_audit_line(
    ...
    task_id = service.create_task("Task A", "")
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-01T00:00:00Z")
    )
    service._append("__app__", "export_checkpoint", {"path": "x.txt"})  # noqa: SLF001
    checkpoint_date = (
    ...
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-10T01:00:00Z")
    )
    service.add_manual_duration(
        task_id, checkpoint_date + timedelta(days=1), 5400, "forgot timer"
    )
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-10T02:00:00Z")
    )
...
def test_tag_sections_include_disclaimer_and_non_exclusive_totals(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    ...
    service.add_manual_duration(
        gamma, datetime(2026, 1, 6, tzinfo=timezone.utc).date(), 1800, "duration"
    )
    service.add_manual_interval(
        delta,
        datetime(2026, 1, 5, 14, 0, tzinfo=timezone.utc).astimezone(),
    ...
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-08T00:00:00Z")
    )
    out = tmp_path / "out.txt"
    service.export_report(out, reset_after=False)
    text = out.read_text(encoding="utf-8")
...
def test_tag_totals_respect_checkpoint_reset_deleted_and_interval_edits(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    ...
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-05T00:00:00Z")
    )
    service.export_report(tmp_path / "first.txt", reset_after=False)
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-06T00:00:00Z")
    )
    service.add_manual_duration(
        task_id, datetime(2026, 1, 6, tzinfo=timezone.utc).date(), 3600, "d"
    )
    service.delete_task(task_id)
    daily, weekly = service.compute_tag_totals(
        parse_utc_z("2026-01-05T00:00:00Z"), parse_utc_z("2026-01-08T00:00:00Z")
    )
...
def test_create_time_submission_marker_appends_event_without_checkpoint_or_reset(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    ...
    service.add_manual_interval(
        task_id,
        datetime(2026, 1, 10, 10, tzinfo=timezone.utc).astimezone(),
        datetime(2026, 1, 10, 11, tzinfo=timezone.utc).astimezone(),
        "r",
    )
    monkeypatch.setattr(
        "task_timer.app.utc_now", lambda: parse_utc_z("2026-01-31T00:00:00Z")
    )
...
def test_export_selected_tasks_report_mark_submitted_behavior(
    ...
    service.export_selected_tasks_report(
        ...
        [t1],
        None,
        parse_utc_z("2026-01-31T00:00:00Z"),
...
def test_selected_export_text_header_and_marking(tmp_path: Path, monkeypatch) -> None:
    ...
    service.add_manual_interval(
        t2,
        datetime(2026, 1, 11, 10, tzinfo=timezone.utc).astimezone(),
        datetime(2026, 1, 11, 11, tzinfo=timezone.utc).astimezone(),
        "r",
    )
    ...
    out = tmp_path / "selected.txt"
    service.export_selected_tasks_report(
        out, [t1], None, end, mark_submitted=True, reason="Job closing"
    )
    text = out.read_text(encoding="utf-8")
...
def test_selected_export_audit_history_is_scoped_to_selected_tasks(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    ...
    service.export_selected_tasks_report(
        ...
        mark_submitted=True,
        reason="Job closing / entered into Epicor",
    )
    text = out.read_text(encoding="utf-8")
...
def test_normal_export_submission_marker_legend_present(
    ...
    service.add_manual_interval(
        t1,
        datetime(2026, 1, 10, 10, tzinfo=timezone.utc).astimezone(),
        datetime(2026, 1, 10, 14, tzinfo=timezone.utc).astimezone(),
        "r",
    )
    service.create_time_submission_marker([t1], None, end, "closing", None)
    monkeypatch.setattr("task_timer.app.utc_now", lambda: end)
...
def test_find_submission_overlaps_cases(tmp_path: Path, monkeypatch) -> None:
    ...
    service.create_time_submission_marker(
        [t1],
        parse_utc_z("2026-01-10T00:00:00Z"),
        parse_utc_z("2026-01-20T00:00:00Z"),
        "closing",
        None,
    )

    no_overlap = service.find_submission_overlaps(
        [t1], parse_utc_z("2026-01-20T00:00:00Z"), parse_utc_z("2026-01-21T00:00:00Z")
    )
    assert no_overlap == []

    same_task_overlap = service.find_submission_overlaps(
        [t1], parse_utc_z("2026-01-15T00:00:00Z"), parse_utc_z("2026-01-16T00:00:00Z")
    )
    assert len(same_task_overlap) == 1
    assert same_task_overlap[0]["task_id"] == t1

    different_task_overlap = service.find_submission_overlaps(
        [t2], parse_utc_z("2026-01-15T00:00:00Z"), parse_utc_z("2026-01-16T00:00:00Z")
    )
    assert different_task_overlap == []


def test_global_export_includes_selected_submitted_task_even_if_deleted_or_reset(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    ...
    service.add_manual_interval(
        ...
        parse_utc_z("2026-05-01T12:10:28Z").astimezone(),
        "r",
    )
    monkeypatch.setattr("task_timer.app.utc_now", lambda: end)
...
def test_parent_export_aggregates_subtasks_and_formats_decimal(
    ...
    service.add_manual_interval(
        parent,
        parse_utc_z("2026-01-10T10:00:00Z").astimezone(),
        parse_utc_z("2026-01-10T11:15:00Z").astimezone(),
        "p",
    )
    service.add_manual_interval(
        child1,
        parse_utc_z("2026-01-10T11:30:00Z").astimezone(),
        parse_utc_z("2026-01-10T14:30:00Z").astimezone(),
        "c1",
    )
    service.add_manual_interval(
        child2,
        parse_utc_z("2026-01-10T15:00:00Z").astimezone(),
        parse_utc_z("2026-01-10T17:10:00Z").astimezone(),
        "c2",
    )
    end = parse_utc_z("2026-01-31T00:00:00Z")
    monkeypatch.setattr("task_timer.app.utc_now", lambda: end)
...
def test_selected_parent_includes_subtasks_and_no_double_count(
    ...
    service.add_manual_interval(
        child,
        parse_utc_z("2026-01-10T11:00:00Z").astimezone(),
        parse_utc_z("2026-01-10T12:00:00Z").astimezone(),
        "c",
    )
    end = parse_utc_z("2026-01-31T00:00:00Z")
    monkeypatch.setattr("task_timer.app.utc_now", lambda: end)
    ...
    service.export_selected_tasks_report(
        out, [parent, child], None, end, mark_submitted=False, reason=""
    )
    text = out.read_text(encoding="utf-8")
...
def test_subtask_alone_selected_does_not_include_parent_and_inherits_parent_tags(
    tmp_path: Path, monkeypatch
) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    parent = service.create_task("Parent", "")
...
def test_depth2_hierarchy_export_selected_tags_and_markers(
    ...
    service.add_manual_interval(
        sub,
        parse_utc_z("2026-01-10T11:00:00Z").astimezone(),
        parse_utc_z("2026-01-10T12:30:00Z").astimezone(),
        "s",
    )
    service.add_manual_interval(
        nested,
    ...
    service.export_selected_tasks_report(
        tmp_path / "sub_selected.txt", [sub], None, end, mark_submitted=False, reason=""
    )
