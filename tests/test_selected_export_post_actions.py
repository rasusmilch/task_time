...
def test_reset_and_delete_selected_tasks_are_scoped_and_append_only(tmp_path) -> None:
    ...
    service.add_manual_interval(
        t1, _local("2026-01-01 10:00"), _local("2026-01-01 11:00"), "seed"
    )
    service.add_manual_interval(
        t2, _local("2026-01-01 10:00"), _local("2026-01-01 11:00"), "seed"
    )

    pre_events = len(service.events)
    service.reset_selected_tasks([t1])
    assert any(
        e["task_id"] == t1 and e["event_type"] == "reset" for e in service.events
    )
    assert not any(
        e["task_id"] == t2 and e["event_type"] == "reset"
        for e in service.events[pre_events:]
    )
    assert not any(
        e["event_type"] == "export_checkpoint" for e in service.events[pre_events:]
    )

    before_delete = len(service.events)
    service.delete_selected_tasks([t1])
    assert any(
        e["task_id"] == t1 and e["event_type"] == "task_deleted"
        for e in service.events[before_delete:]
    )
    assert not any(
        e["task_id"] == t2 and e["event_type"] == "task_deleted"
        for e in service.events[before_delete:]
    )
...
def test_reset_selected_tasks_returns_only_changed_and_keeps_task(tmp_path) -> None:
    ...
    assert any(
        e["task_id"] == parent and e["event_type"] == "reset" for e in service.events
    )
    assert any(
        e["task_id"] == child and e["event_type"] == "reset" for e in service.events
    )
