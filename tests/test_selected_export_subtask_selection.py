...
def test_selected_subtask_alone_exports_only_subtask(tmp_path) -> None:
    ...
    rows = service.compute_selected_task_totals([child], None, _local("2026-01-02 00:00").astimezone(timezone.utc))
    assert [r["task_id"] for r in rows] == [child]


def test_selected_parent_and_child_are_normalized_no_double_count_and_marker_records_inclusion(tmp_path) -> None:
    service = TaskTimerService(EventStorage(tmp_path))
    parent = service.create_task("P", "")
    child = service.create_subtask(parent, "C", "")
    service.add_manual_interval(parent, _local("2026-01-01 10:00"), _local("2026-01-01 11:00"), "p")
    ...
    service.export_selected_tasks_report(output, [parent, child], None, end, mark_submitted=True, reason="done")
